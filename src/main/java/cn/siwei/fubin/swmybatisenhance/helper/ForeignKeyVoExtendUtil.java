                ...
                if (!ObjectUtils.isEmpty(VoExtendAno)) {
                    ...
                    if (idValue instanceof List) {
                        ...
                        if (type.equals(VoExtendType.OBJECT)) {
                            ...
                            //获取字段名称
                            String extName = VoExtendAno.foreignModelFieldForName();
                    ...
                    if (!ObjectUtils.isEmpty(selfIdSet)) {
                        try {
                            ...
                            for (Object extItem : extModelList) {
                                ...
                                String selfIdValueInExtModelKey = getSelfIdValueInExtModelMethod.invoke(extItem).toString();
                                if (type.equals(VoExtendType.Str)) {
                                    //获取得到name值
                                    String getMethodName = StringUtils.getGetMethodName(extName);
                                    Method method = extModelClass.getMethod(getMethodName);
                                    Object nameValue = method.invoke(extItem);
                                    List<Object> objects = strMap.getOrDefault(selfIdValueInExtModelKey,null);

                                    if (objects==null) {
                                        objects = new ArrayList<Object>();
                                        objects.add(nameValue);
                                        strMap.put(selfIdValueInExtModelKey, objects);
                                    } else {
                                        objects.add(nameValue);
                                    }
                                }
                                if (type.equals(VoExtendType.OBJECT)) {
                                    //这部分用到了引用，直接add即可。
                                    List<Object> extModelListForMap = objMap.getOrDefault(selfIdValueInExtModelKey, null);
                                    if (extModelListForMap==null) {
                                        extModelListForMap = new ArrayList<Object>();
                                        extModelListForMap.add(extItem);
                                        objMap.put(selfIdValueInExtModelKey, extModelListForMap);
                                    } else {
                                        extModelListForMap.add(extItem);
                                    }
                                }
                            ...
                            for (Object modelObj : extModelList) {
                                ...
                                if (!ObjectUtils.isEmpty(extModelId)) {
                                    for (Object key : selfId_extId_Map.keySet()) {
                                        //分组后的list
                                        List<Object> modelIdList = selfId_extId_Map.getOrDefault(key, Collections.emptyList());
                                        if (modelIdList.size() != 0) {
                                            if (modelIdList.contains(extModelId)) {
                                                if (type.equals(VoExtendType.Str)) {
                                                    ...
                                                    List<Object> extIdList = strMap.getOrDefault(key.toString(), null);
                                                    if (extIdList==null) {
                                                        extIdList = new ArrayList<Object>();
                                                        extIdList.add(nameValue);
                                                        strMap.put(key.toString(), extIdList);
                                                    } else {
                                                        extIdList.add(nameValue);
                                                    }
                                                ...
                                                if (type.equals(VoExtendType.OBJECT)) {
                                                    List<Object> extModelListForMap = objMap.getOrDefault(key.toString(), null);
                                                    if (extModelListForMap==null) {
                                                        extModelListForMap = new ArrayList<Object>();
                                                        extModelListForMap.add(modelObj);
                                                        objMap.put(key.toString(), extModelListForMap);
                                                    } else {
                                                        extModelListForMap.add(modelObj);
                                                    }
                                                }
